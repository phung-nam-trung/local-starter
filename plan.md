...
- [ ] **TA1: Khởi tạo Electron + Vite + React, mở được cửa sổ rỗng**
  ...
  - Deps: —
- [ ] **TA2: Repo registry + IPC bridge khung**
...
- [ ] **TB1: Git layer — list branch + current + isClean**
  ...
  - Deps: TA2
- [ ] **TB2: Fetch + checkout + pull an toàn + branch picker UI**
...
- [ ] **TC1: Install deps khi thiếu (npm theo repo sp; pnpm ở root new-frontend)**
...
### Phase D — VPN (F5)
- [ ] **TD1: VPN detect → mở openvpn-gui → poll + thông báo + skip**
...
- [ ] **TE1: Chọn prod/test → `.env` (backup, đảm bảo clients_dir, không log secret)**
...
- [ ] **TF1: selfpointrest — install→buildAll→start (3000) + build UI backend/frontend tùy chọn**
...
- [ ] **TF3: mobile / collection (9000) + stor-web (3002)**
...
- [ ] **TG1: Hỗ trợ sửa `test/products.js` & `test/specials.js` + restart**
