...
### Tasks
- [ ] **TM1: `vpn.js` — adapter là cách detect DUY NHẤT (bỏ TCP probe)**
...
- [ ] **TM2: `VpnStatus.jsx` — bỏ ô Probe host/Port; Connect = mở OpenVPN app + poll adapter**
...
- [ ] **TM3: `ipc.js` + `store.js` — gỡ `probeHost/probePort`**
...
- [ ] **TM4: Docs — CONTEXT §9 + LAUNCHER + header comments**
...
- [ ] Detect VPN chỉ dựa **adapter Up** per-OS; không còn TCP-probe / ô nhập host:port; `probe` không còn là code path.
- [ ] Nút **Connect** mở app OpenVPN (win `openvpn-gui.exe` mặc định) để user tự authen; poll adapter tới khi Up.
- [ ] `build:renderer` + `node --check` PASS; config cũ (`probeHost/probePort`) không gây lỗi.
- [ ] Docs (CONTEXT §9/F5 + LAUNCHER) phản ánh adapter-based, bỏ probe DB/ES.
