...
>   2. `LAUNCHER.md`: thêm bước onboarding chọn 2 workspace root; prerequisites theo OS (VPN client: win OpenVPN GUI · mac Tunnelblick · linux `nmcli`/`openvpn3`; `corepack enable` cho pnpm); cách đóng gói `electron-builder --win|--mac|--linux`.
>   3. `README.md` nếu cần.
